...
class XenAPI {
    ...
    */
    public function __construct() {
        ...
        require_once($this->xfDir . '/src/XF.php');
        XF::start($this->xfDir);

        //XenForo_Autoloader::getInstance()->setupAutoloader($this->xfDir. '/library');
        XF::app()->initialize($this->xfDir . '/src', $this->xfDir);
        XF::app()->set('page_start_time', microtime(TRUE));

        $deps = new XenForo_Dependencies_Public();
        $deps->preLoadData();

        // Disable XenForo's PHP error handler.
        XF::app()->disablePhpErrorHandler();
